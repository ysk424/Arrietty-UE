...
飛行モードではT2の表示速度を移動速度にせず、測定パワーをプロペラ効率80%で推進力へ変換します。有効質量35 kg、翼面積15 m2、空気密度1.225 kg/m3、滑空比30、最良滑空速度24 km/hを基準に、動圧、揚力係数、誘導抗力を計算します。対気速度は倍率を掛けず、物理計算、水平移動、計器表示のすべてで同じ値を使います。抗力を上回る正の余剰出力だけは既定x10のパワーブーストを適用し、負の出力収支は増幅しません。飛行チューニング中は測定パワーを固定テスト推進Wへ置換し、推進W、パワーブースト、ピッチ応答、ロール応答を順番に変更します。

翼取付角と`Pitch - FPA`から迎角を求め、揚力が重量を超え、対気速度20 km/h以上かつ正の機首上げ目標なら離陸します。18 km/h未満または限界迎角超過で失速し、機首下げ目標かつ20.5 km/h以上で回復します。公開するロール目標は右を正とし、内部バンク角へ符号変換します。バンクした揚力は`Lift*cos(bank)`を鉛直成分、`Lift*sin(bank)`を旋回成分として運動へ適用し、ハンドルのラダー旋回率も加えます。空中で地上モードへ直接切り替えることはできず、`SecretWorldRideSurface`上へ着陸してから切り替えます。

Button 3/4とJoystick 2は同じピッチ／右ロール目標値を更新し、その物理角をシミュレーションへ直接渡します。ピッチ／バンクの目標角は1°刻みで安全上限±12°／±25°を維持し、そこへ到達する角速度だけを`(airspeed / 24 km/h)^2`で変えます。通常の操舵効率は0.20〜1.75へclampし、失速中は25%へ落とします。FPAは力の鉛直釣合いから積分し、AoAは`wing incidence + Pitch - FPA`として計器とCSVへ出します。通常時はT2の実測パワー、明示的な飛行チューニング中だけ固定テスト推進Wを使います。

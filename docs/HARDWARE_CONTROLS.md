...
| Button 5 | 飛行中の推進パワー`x1`／`x5`トグル。T2実測Wは変更せず、飛行モデルへ渡す推進Wだけを5倍にする。地上・停止時は`x1`へ戻る |
...
降下エネルギーが対気速度へ戻る。操舵の到達速度は対気速度の二乗（動圧）に応じて変わり、24 km/hで基準値1.0、低速で鈍く、高速で最大1.75となる。失速中は通常値の25%へ落とし、最低2.0 m/sで降下させる。Button 5の`x5`は離陸補助・地球移動用の意図的なエンジンブーストであり、計器とCSVには実測Wと推進Wを別々に表示・記録する。
...
Button 5=`GPIO23`だけは直読でも検出されず、実物のButton 5側の半田切れを確認した。これはソフトウェアでは直せないため、半田修理まではButton 5を未使用にする。診断時点では作業のためボタンコネクターを外してある。USB電源を切った状態で修理・再接続する。

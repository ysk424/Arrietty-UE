...
現時点のArrietty操作割り当ては次のとおりとする。2台のJoystickのX/Y/SWは入力として報告・表示するが、
ゲーム内用途を決めるまではすべて未割り当てとする。

| 入力 | Arrietty操作 |
|---|---|
| Button 1 | 未開始時: Start Ride／開始ライン方向へHMD仮想視界を合わせる。開始後: 実際の走行軌跡を約2 m戻る安全復帰 |
| Button 2 | 地上／飛行モード切替 |
...
| Joystick 2 X/Y/SW | 用途未定。入力値のみ報告・表示 |

走行中のカーブはハンドル操舵を現状維持し、Joystick入力とは結び付けない。

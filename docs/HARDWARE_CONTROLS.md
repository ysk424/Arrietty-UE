...
| Joystick 1 SW | 飛行中に調整を開始。押すたび現在値を確定し、固定推進W→飛行速度倍率→正の上昇倍率→ピッチ応答→最大上下動→ロール応答へ進み、最後の確定で終了 |
...
人力飛行モデルの初期値は、有効質量35 kg、滑空比30、プロペラ効率80%、エネルギー計算上の最良滑空速度24 km/h、
離陸20 km/h、失速18 km/h、失速回復20.5 km/hとする。既定の飛行速度倍率3.0により、計器と世界移動ではそれぞれ72、60、54、61.5 km/hとして扱う。約95 Wで水平飛行になり、
...
飛行チューニングはJ1 SWで開始し、調整中だけT2実測Wの代わりに固定推進力を使う。初期値と1ジェスチャーの刻みは、推進95 W／5 W、飛行速度倍率3.0／0.5、正の上昇倍率10／1、ピッチ応答28 deg/s／2 deg/s、最大上下動1.5 m/s／0.1 m/s、ロール応答55 deg/s／5 deg/sである。上昇倍率は抗力を上回る正の余剰出力だけに適用し、出力不足や無動力沈下には適用しない。140 W、エネルギー速度24 km/h、中立姿勢では約1.04 m/s、300 mまで約5分を初期目安とする。各値は`Saved/arrietty_ride.csv`へ記録され、最後の項目を確定すると実測パワーへ戻る。

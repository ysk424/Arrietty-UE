...
| Joystick 1 X | 飛行調整中、中央から左／右へ1回倒して戻すたび、選択値を1段階減少／増加。倒しっぱなしでは反復しない |
| Joystick 1 Y | 現在は入力値の報告・表示のみ |
| Joystick 1 SW | 飛行中に調整を開始。押すたび現在値を確定し、固定推進W→正の余剰パワーブースト→ピッチ応答→ロール応答へ進み、最後の確定で終了 |
...
人力飛行モデルの初期値は、有効質量35 kg、翼面積15 m2、空気密度1.225 kg/m3、滑空比30、プロペラ効率80%、最良滑空速度24 km/h、
離陸20 km/h、失速18 km/h、失速回復20.5 km/hとする。計器と世界移動は倍率なしの実対気速度を使い、約95 Wで24 km/h水平飛行になる。
翼取付角と`Pitch - FPA`から迎角を求め、動圧と揚力係数から揚力、揚力係数から誘導抗力を計算する。ロール時は揚力を鉛直成分と旋回成分へ分ける。
操舵の到達速度は対気速度の二乗（動圧）に応じて変わり、24 km/hで基準値1.0、低速で鈍く、高速で最大1.75となる。失速中は通常値の25%へ落とす。ピッチ目標は±12°、右ロール目標は±25°でclampする。旧Button 5の`x5`ブーストは廃止した。

飛行チューニングはJ1 SWで開始し、調整中だけT2実測Wの代わりに固定推進力を使う。初期値と1ジェスチャーの刻みは、推進95 W／5 W、正の余剰パワーブースト10／1、ピッチ応答28 deg/s／2 deg/s、ロール応答55 deg/s／5 deg/sである。パワーブーストは抗力を上回る正の余剰出力だけに適用し、出力不足や無動力沈下には適用しない。各値は`Saved/arrietty_ride.csv`へ記録され、最後の項目を確定すると実測パワーへ戻る。

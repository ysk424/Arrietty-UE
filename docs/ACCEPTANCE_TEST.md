...
9. Joystick 1/2のSWを押してbutton maskのbit 6/7を確認する。Joystick 2 SWでピッチ／右ロール目標が両方0°へ戻ることを確認する。飛行中にJoystick 1 SWを押すと`TUNE 1/6 TEST PROP 95 W`となり、Joystick 1を右へ1回倒して戻すと100 W、左で95 Wへ戻ることを確認する。倒しっぱなしでは反復しないことを確認する。SWを押すたび`SPEED x3.0`、`CLIMB x10`、ピッチ応答、最大上下動、ロール応答へ進み、速度倍率は0.5、上昇倍率は1ずつ変わること、最後の確定で`TUNE OFF`と実測推進力へ戻ることを確認する。
...
8. テンキー7またはButton 2で人力飛行モードにし、既定速度倍率3.0では計器が60 km/h以上を示す状態で機首上げ目標を+1°以上にすると離陸する。エネルギー等価24 km/h（表示72 km/h）を基準に、低速ではピッチ／バンクの到達が鈍く、高速では速くなるが最大ピッチ±12°／最大バンク±25°を超えない。140 W前後・中立姿勢でV/Sが約+1.0 m/sとなり、約5分でALT AGL 300 mへ到達できること、その後0 Wで沈下率が約-0.22 m/sとなり長く滑空できることを確認する。ハンドルは地上の前輪操舵ではなく飛行中のラダーとしてヨーを動かす。表示54 km/h未満で`STALL`警告が出て、機首下げ目標かつ表示61.5 km/h以上にすると回復する。コース外へ飛べるが、空中で地上モードへ切り替えられず、`SecretWorldRideSurface`上へ着陸後に切り替えられる。
9. 緑色の外枠を持つ不透明な54×30cmの計器が仮想自転車の正面1.25 m、中心高1.30 mに表示される。頭を右へ振ると計器は視界の左へ移るが、板自体はHMDへ追従回転せず、仮想自転車の進行方向に固定される。
10. 計器に`ON GROUND`／`AIRBORNE`、対気速度を主表示する人工水平儀、心拍、ケイデンス、実測／推進パワー、距離、時刻、滑走路基準の`ALT AGL`、昇降率、Heading、Pitch、Bank、FPA、AoA、操舵効率、失速／過速度状態が表示される。Cesium Levelではlongitude、latitude、別値として`ELLIP H`も表示される。心拍計がない場合もT2走行は継続する。

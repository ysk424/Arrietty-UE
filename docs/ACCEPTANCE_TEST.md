...
3. HMDで正面を向き、ハンドル中央でテンキー0を押し、そのまま1秒保つ。HMD/BikeのUE Forwardが一致し、`INSTRUMENT ANCHORED / STEM TRACKER`が短時間表示され、T2制御が`FLAT P5`になって開始音が鳴る。直進時のRaw/Applied操舵角が0°付近で、Heading 0°ならUE +Xへ直進する。
...
9. 緑色の外枠を持つ不透明な54×30cmの計器が表示され、下辺中央がステムの右コントローラー位置付近にある。トラッカーを動かすと滑らかに追従し、追跡を隠した間は最後の位置を保持する。
